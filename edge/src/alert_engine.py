...
class AlertEngine:
    ...
    def _monitoring_loop(self, interval: int):
        ...
        while self._monitoring_active:
            try:
                ...
                # Publicar estado de alertas activas cada ciclo
                self._publish_active_alerts_to_thingsboard()
    ...
    def _emit_alert(self, alert_data: Dict[str, Any]):
        ...
        try:
            ...
            # Publicar estado actualizado de alertas activas
            # Extraer unit_id del sensor_id
            sensor_id = alert_data.get('sensor_id', '')
            if sensor_id and sensor_id.startswith('UNIT_'):
                parts = sensor_id.split('_')
                if len(parts) >= 2:
                    try:
                        unit_id = int(parts[1])
                        self._publish_active_alerts_to_thingsboard(unit_id)
                    except ValueError:
                        pass
    ...
    def _publish_active_alerts_to_thingsboard(self, unit_id: Optional[int] = None):
        """
        Publica el resumen de alertas activas como atributos de ThingsBoard.
        
        Args:
            unit_id: ID del dispositivo (opcional, si None publica para todos)
        """
        ...
        try:
            ...
            active_alerts = self.db.get_alerts(ack=False, limit=500)
            ...
            for alert in active_alerts:
                ...
                # Extraer unit_id del sensor_id (UNIT_2_TILT_X -> 2)
                if sensor_id and sensor_id.startswith('UNIT_'):
                    parts = sensor_id.split('_')
                    if len(parts) >= 2:
                        try:
                            u_id = int(parts[1])
                            if unit_id is None or u_id == unit_id:
                                if u_id not in alerts_by_unit:
                                    alerts_by_unit[u_id] = []
                                alerts_by_unit[u_id].append(alert)
                        except ValueError:
                            continue
            
            # Si se especificó unit_id pero no tiene alertas, publicar estado vacío
            if unit_id is not None and unit_id not in alerts_by_unit:
                alerts_by_unit[unit_id] = []
            
            # Publicar atributos para cada dispositivo
            for u_id, alerts in alerts_by_unit.items():
                device_name = f"Sensor_Unit{u_id}"
                
                # Contar alertas por nivel
                count_by_level = {'INFO': 0, 'WARN': 0, 'ALARM': 0, 'CRITICAL': 0}
                alert_messages = []
                
                for alert in alerts:
                    level = alert.get('level', 'INFO')
                    count_by_level[level] = count_by_level.get(level, 0) + 1
                    alert_messages.append(f"{alert.get('code')}: {alert.get('message')}")
                
                # Publicar como atributos
                attributes = {
                    'active_alerts_count': len(alerts),
                    'active_alerts_critical': count_by_level['CRITICAL'],
                    'active_alerts_alarm': count_by_level['ALARM'],
                    'active_alerts_warn': count_by_level['WARN'],
                    'active_alerts_info': count_by_level['INFO'],
                    'active_alerts_list': ' | '.join(alert_messages[:5]) if alert_messages else 'No active alerts'
                }
                
                self.mqtt_bridge.publish_device_attributes(device_name, attributes, force=True)
                logger.debug(f"📊 Alertas activas publicadas para {device_name}: {len(alerts)} total")
            
            # Publicar resumen global en el Gateway (RPI_EDGE) para vista agregada
            if active_alerts:
                total_count = len(active_alerts)
                global_count_by_level = {'INFO': 0, 'WARN': 0, 'ALARM': 0, 'CRITICAL': 0}
                all_messages = []
                
                for alert in active_alerts:
                    level = alert.get('level', 'INFO')
                    global_count_by_level[level] = global_count_by_level.get(level, 0) + 1
                    sensor_id = alert.get('sensor_id', 'N/A')
                    all_messages.append(f"[{sensor_id}] {alert.get('code')}: {alert.get('message')}")
                
                gateway_attributes = {
                    'active_alerts_total': total_count,
                    'active_alerts_critical': global_count_by_level['CRITICAL'],
                    'active_alerts_alarm': global_count_by_level['ALARM'],
                    'active_alerts_warn': global_count_by_level['WARN'],
                    'active_alerts_info': global_count_by_level['INFO'],
                    'active_alerts_devices': len(alerts_by_unit),
                    'active_alerts_summary': ' | '.join(all_messages[:10]) if all_messages else 'No active alerts'
                }
                
                self.mqtt_bridge.publish_device_attributes('RPI_EDGE', gateway_attributes, force=True)
                logger.debug(f"📊 Resumen global de alertas publicado en RPI_EDGE: {total_count} total de {len(alerts_by_unit)} dispositivos")
            else:
                # No hay alertas activas, publicar atributos en cero
                gateway_attributes = {
                    'active_alerts_total': 0,
                    'active_alerts_critical': 0,
                    'active_alerts_alarm': 0,
                    'active_alerts_warn': 0,
                    'active_alerts_info': 0,
                    'active_alerts_devices': 0,
                    'active_alerts_summary': 'No active alerts'
                }
                self.mqtt_bridge.publish_device_attributes('RPI_EDGE', gateway_attributes, force=True)
                logger.debug("📊 Resumen global: Sin alertas activas")
        
        except Exception as e:
            logger.error(f"Error al publicar alertas activas a ThingsBoard: {e}", exc_info=True)
    ...
    def _auto_acknowledge_alert(self, alert_id: int, sensor_id: Optional[str], code: str, reason: str):
        ...
        try:
            ...
            # Actualizar estado de alertas activas en ThingsBoard
            if sensor_id and sensor_id.startswith('UNIT_'):
                parts = sensor_id.split('_')
                if len(parts) >= 2:
                    try:
